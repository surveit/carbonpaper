...
**Status:** design approved; implementation starting on the runtime substrate (§4).
Revised 2026-07-02 after discussion: lineage is now runtime-tracked at execution
time, replacing an earlier draft that reconstructed lineage from per-stage key
declarations (that draft leaned on "join keys" as if it were a pipeline-wide enforced
concept; it isn't — see §3).
**Branches:** the runtime lineage substrate (§4) and tracer (§5) target master on
`syw-lineage` (worktree `prototype_one_lineage_wt`), now that PR #26's row/frame split
is merged (PR #41). The palm dossier renderer (§6.3 static) stays on `palm-on-master`
(worktree `prototype_one_syw_wt`) where the example lives. The design docs live in
both.
...
- **PR #26 (`python-row-frame-functions`) is the substrate — merged (as PR #41).** Its
  row/frame split is what makes most python stages' lineage recordable rather than
  recovered. Lineage recording touches the same handlers; the runtime part of this work
  now branches off master (`syw-lineage`), since #26 has landed.
