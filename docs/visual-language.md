...
action: a version snapshots whatever the author has, and an undescribed stage, one that
says nothing about what its output rows are, one no example checks, or a model call that
re-rolls every run are all things an author may knowingly leave standing. A compiler note
borrowing `error` claimed a severity it could not act on.
