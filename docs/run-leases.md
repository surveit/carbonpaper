...
`app/runtime/lease.py` claims a lease before a run executes and releases it after.
`app/core/sqlite_store.py` stores one row per run in the `run_lease` table and owns
...
|---|---|---|
| live | someone is executing it | left alone |
