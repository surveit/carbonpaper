...
window.Canvas = window.Canvas || (function(){
  ...
  async function openPanel(view, kind, id, tab){
    view.picked = {kind, id};
    markPicked(view);
  ...
  function fetchPanel(view, id){
    if(!view.panels[id]){
      const query = new URLSearchParams({
        stage: view.nav.cited_stage, row: view.pane.dataset.row, column: view.nav.column});
      view.panels[id] = fetch(`${runBase(view)}/stage/${encodeURIComponent(id)}/traced?${query}`)
        .then(answer => answer.ok ? answer.text()
          : `<p class="muted">could not load ${esc(id)} (${answer.status})</p>`)
        .catch(failure => {
          delete view.panels[id];
