...
this is prose → named schemas. It builds an `app.core.agent.Agent` whose target schema is
...
engine, and is the bridge onto it: app.core.agent is a core module open to every layer, so the higher-level
orchestration in app.services (generation) delegates here rather than reaching into the
engine itself. The submitted SchemaLibrary is handed back through a callback; persisting it
is the caller's job.
"""
