...
NODE_TYPES: dict[str, dict[str, _Any]] = {
    ...
    "publish": {
        ...
        "notes": (
            ...
            "from_file=<the file you are writing>, row=<the row you are rendering>)`, "
            ...
            "relative to it, and it must live under output_dir. Iterate however you like "
            "and sort, rank, or filter freely — each row carries its own provenance in a "
            "`trace_row_ordinal` column the runtime adds, so just pass the row itself. Do "
            "not build the row you pass by hand or drop that column; a row without it "
            "raises. A row whose lineage "
