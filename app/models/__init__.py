...
NODE_TYPES: dict[str, dict[str, _Any]] = {
    ...
    "publish": {
        ...
        "notes": (
            ...
            "`trace_links.export_row_trace(\"<the input stage's id>\", row_ordinal, "
            "from_file=<the file you are writing>)`, which WRITES that row's trace page "
            "under output_dir and returns a relative href to embed. Pass the path of the "
            "file you are writing: the href is relative to it, and it must live under "
            "output_dir. row_ordinal is that row's 0-based position in the input frame AS "
            "RECEIVED. Iterate the frame in order (enumerate it) and do not sort, filter, "
            "or dedup before reading the ordinal — position is the only key the trace has, "
            "so a reordered frame yields confidently wrong links. A row whose lineage "
