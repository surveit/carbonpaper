...
def validate_named_schema(schema: dict[str, Any], where: Optional[str] = None) -> list[str]:
    """Non-fatal: validate ONE named schema dict (a TableSchema + name/kind/
    references), returning human-readable issues ([] means valid). Cross-schema
    reference resolution is checked library-wide by validate_schema_library.

    `where` prefixes each issue (the caller's label for this schema); when omitted
    it defaults to `schema `<name>`` so a standalone issue is still locatable."""
    label = where or (f"schema `{schema.get('name')}`" if schema.get("name") else "schema <no-name>")
    try:
        NamedSchema.model_validate(schema)
        return []
    except ValidationError as err:
        return [f"{label}: {msg}" for msg in format_errors(err)]
