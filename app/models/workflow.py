...
def validate_edge_schemas(stages: list[Stage]) -> list[str]:
    ...
    naming only the columns the stage consumes — that the upstream's resolved
    output schema must subsume (matching spec, compatible nullability, not
...
def validate_workflow(stages: list[Stage]) -> list[str]:
    ...
    schema supplied by its upstream's resolved output. Per-stage invariants (e.g. llm_transform being strictly 1:1) are
