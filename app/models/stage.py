...
class PythonFunction(_Base):
    ...
    code: Optional[str] = Field(
        ...
        description=(
            ...
            "`trace_links.export_row_trace(stage_id, from_file=<the file you are writing>, "
            "row=row)` — it writes that row's trace page under output_dir and returns a "
            "relative href to embed. There is no ordinal to pass: the runtime adds a "
            "`trace_row_ordinal` column to each input frame, so sort, rank, or filter "
            "however the report needs and just hand back the row. A row built by hand, or "
            "one that column was dropped from, raises. A row whose lineage "
