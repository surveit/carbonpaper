...
class PythonFunction(_Base):
    ...
    code: Optional[str] = Field(
        ...
        description=(
            ...
            "`trace_links.export_row_trace(stage_id, row['trace_row_ordinal'], "
            "from_file=<the file you are writing>, row=row)` — it writes that row's trace "
            "page under output_dir and returns a relative href to embed. Declaring "
            "`trace_links` makes the runtime add a `trace_row_ordinal` column carrying each "
            "row's POSITION in the stage output; read the ordinal from it, never from "
            "enumerate(), so sorting or filtering before rendering still links every claim "
            "to its own row. The exporter raises if `row` and the ordinal disagree. "
            "A row whose lineage "
