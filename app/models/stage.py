...
class ConnectorKind(str, Enum):
    # Full connector vocabulary the compiler may emit and the prompt advertises to
    # the LLM. Only `file` and `computed_static` are runnable today (see
    # IMPLEMENTED_CONNECTOR_KINDS in app.models); the rest are valid contract-wise
    # but raise NotImplementedError at run time (handlers.handle_input_data).
    file = "file"
    http = "http"
    scrape = "scrape"
    api = "api"
...
class AggFormula(str, Enum):
    ...
    first = "first"
    list = "list"
    # Weighted formulas need a value_column + weight_column (checked below and by
    # runtime/handlers.py). Kept in the contract so the runtime can execute them.
...
def validate_table_schema(schema: dict[str, Any] | None, where: str) -> list[str]:
    """Structural validation of an inline TableSchema dict (an input `schema` or an
    `output_schema`), returning human-readable issues ([] means valid).

    The Pydantic `TableSchema`/`Column` models validate an `output_schema` when a
    Stage is constructed, but a Stage's INPUT schemas are stripped to bare ids by
    `Stage._ids_only` (inputs are just upstream references). This function is how
    the validator FUNCTIONS re-check those input schemas so the accept/reject
    decision matches the whole-stage contract — and it emits the same messages."""
    issues: list[str] = []
    if schema is None:
        return issues
...
def _input_schema_issues(stage: dict[str, Any]) -> list[str]:
    """Validate the inline `schema` on each of a stage's inputs (the part the Stage
    model discards). Prefix matches the whole-stage contract's wording."""
    sid = stage.get("id") or "<no-id>"
    issues: list[str] = []
...
def validate_stage(stage: dict[str, Any]) -> list[str]:
    ...
    input_issues = _input_schema_issues(stage)
    try:
        Stage.model_validate(stage)
        return input_issues
    except ValidationError as err:
        return format_errors(err) + input_issues
