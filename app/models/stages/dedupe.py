"""dedupe stage: the config block, plus column validation — `keys` and the
tie-breaking `by` column must resolve against the input, and the signature's
`produces` must be satisfied by that input, since every surviving row is an
input row unchanged."""
...
class DedupeStage(AbstractStage):
    ...
    def find_signature_schema_issues(
        ...
        return find_dedupe_signature_issues(self, inputs)
...
def find_dedupe_signature_issues(
    stage: "DedupeStage", inputs: Sequence["WorkflowStageInput"]
) -> list[str]:
    """A survivor is an input row untouched, so its input must satisfy every produced column."""
    produced = TableSchema(columns=stage.signature.produces)
    return [
        f"stage '{stage.id}': signature produces vs input `{inputs[0].id}` — {reason}"
        for reason in produced.find_unsatisfied_columns(inputs[0].table_schema)
    ]
