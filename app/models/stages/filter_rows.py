"""filter_rows stage: the config block. It keeps a subset of its single input's
rows unchanged, so its signature declares reads only — never adds or rewrites."""
...
class FilterRowsStage(StageBase):
    ...
    def find_signature_config_issues(self) -> list[str]:
        if self.signature.adds or self.signature.rewrites:
            return [
