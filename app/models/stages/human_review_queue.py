...
REVIEW_PREDICATE_DESCRIPTION = (
    "What the person at this step did, as a verb phrase completing \"each ___ before it "
    "counted\" — \"went in front of a person\", \"was read against the filing text by a "
    "reviewer\". Lower case, no full stop. It labels this step wherever a reader meets "
    "it, so it says what the review MEANT for the rows, never how the queue is "
    "configured. Say only what this step guarantees: who actually signed a given row is "
    "in the run's own reviewer column, and a claim about that belongs there rather than "
    f"here. HARD LIMIT: {PREDICATE_MAX_CHARS} characters, refused above that."
)
