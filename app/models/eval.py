...
from app.models.workflow import Workflow, executable_frontier
...
def resolve_eval_run_settings(
    ...
    """Decide whether the override→target path can be scored automatically
    row-by-row (every frontier stage grain-preserving) — the v1 condition for a
    single-table, row-aligned eval.

    The frontier itself (the stages that execute to produce `target` given
    `overrides`) is derived by `executable_frontier` — the same graph walk
    `app.runtime.runner.run_subset` uses to derive what IT executes, so the two
    can never disagree. This function adds the eval-specific judgment on top:
    whether every frontier stage preserves grain.
    ...
    frontier = executable_frontier(workflow.stages, [target], overrides)
    ...
    return EvalRunSettings(can_score_declaratively=not blocking,
                           frontier=frontier, blocking_stages=blocking)
