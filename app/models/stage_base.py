...
class StageCommon(_Base):
    ...
    # reshaping family — app.models.stages.signature) and REQUIRES it: the
    # stage's output schema resolves from nothing else (resolve_output_schema).
    # The draft keeps the permissive optional union, per the StageDraft
    # philosophy. It is checked against the config (find_signature_config_issues)
    # and the edges (find_signature_issues).
...
class StageBase(StageCommon):
    ...
    # everything else must promise output columns through its signature.
    ...
    def resolve_output_schema(self) -> Optional[TableSchema]:
        """The output schema the signature promises; None only when it promises no table."""
    ...
    def compute_definition_fingerprint(self) -> str:
        ...
        assert self.signature is not None  # required on every stored per-type model
        fields: dict[str, Any] = {
            "type": self.type,
    ...
    def _schemas_declared(self) -> "StageBase":
        ...
        if self.signature is None:
            issues.append("declares no signature")
