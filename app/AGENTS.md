...
offered on the version page, which is where one is stored. The work column opens with the
toolbar below, then four named sections, in this order:
...
Above them, the **toolbar** (`.run-toolbar`), right-aligned: the review-packet export as a
link, and beside it a menu button holding the raw manifest and the whole-run log
(`.run-audit-menu`, closed on load, and closed again by Escape or a click outside it).
