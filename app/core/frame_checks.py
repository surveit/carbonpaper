...
def _find_duplicate_row_groups(rows: Sequence[Row]) -> list[list[int]]:
    ...
    for pos, row in enumerate(rows):
        # Sorted so key order cannot change identity, and repr() rather than str()
        # so cells of different types with the same face value ("1" vs 1) stay
        # distinct. The digest never leaves this function — it is not the stage
        # cache's fingerprint, so what it renders is free to change.
