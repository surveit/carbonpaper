...
import graph and imports nothing first-party at all — guarded by the
import-linter contract in ``pyproject.toml``.

``app.core.sqlite_store`` provides the concrete SQLite backend; ``app.main``
wires one to the other with a single ``configure_store()`` call at startup.
Swapping the backend (Postgres, or plain files for inspection) is a new
``DocumentStore`` implementation plus that one call; nothing above the seam
changes.
