...
def resume_run(
    ...
    # The recorded parameters carry the run's KIND as well as its settings, and the
    # two must be built together: a test run's auto-approve is only legal against a
    # read-only stage cache, so pairing it with the production context's writable one
    # is refused by RunContext and the resume dies before its first stage.
