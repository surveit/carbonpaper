"""A run's own copy of the ReviewDecision rows it was handed. docs/run-manifest.md"""
...
def _find_review_decisions_file(run_dir: Path, stage_id: str) -> Path | None:
    for suffix in (".parquet", ".csv"):
        path = run_dir / REVIEW_DECISIONS_DIR / f"{stage_id}{suffix}"
        if path.exists():
            return path
    return None
