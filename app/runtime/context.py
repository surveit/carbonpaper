...
`app.runtime.runner.prepare_run`/`resume_run`), by `RunContext.for_non_production`
...
auto-approving a production review queue.

`identity`/`stage_cache` are the pair a caller chooses at construction:
`for_production` grants both (this run's (project, run_id) and a read+write
...
class RunContext(BaseModel):
    """Immutable identity + config for one run. `for_production` sets
    `mode="production"` and grants project scope (`identity` + a read+write
    stage-result cache); `for_non_production` sets `mode="non_production"`, may
    ...
    def _production_forbids_queue_auto_approve(self) -> RunContext:
        if self.mode == "production" and self.queue_auto_approve:
            raise ValueError(
                "queue_auto_approve is a non-production bypass; a production run "
