...
class RowTraceExporter:
    ...
    def export_row_trace(self, stage_id: str, row_ordinal: int, from_file: Path) -> str:
        ...
        if not page.is_file():
            ...
            copy_trace_assets(self.output_dir / _ASSETS_DIR)
        return _relative_href(page, from_file.parent)

    def _locate_page(self, stage_id: str, row_ordinal: int) -> Path:
        # quote() keeps a stage id carrying separators or `..` inside the bundle.
        return self.output_dir / _TRACES_DIR / quote(stage_id, safe="") / f"{row_ordinal}.html"
    ...
    def _write_page(self, page: Path, view: dict[str, Any]) -> None:
        ...
        prefix = _relative_href(self.output_dir / _ASSETS_DIR, page.parent) + "/"
...
__all__ = ["RowTraceExporter", "RowTraceLinker"]
