...
_PLACE = (
    "YOUR PLACE. Six reviewers have just read ONE sentence a journalist proposes to "
...
_HANDED = (
    "WHAT YOU ARE HANDED. The evidence pool, in the blocks the reviewers read it in — "
    ...
    "with the reviewer that wrote it. The grounding reviewer returned phrases with "
    "offsets rather than challenges. The meaning reviewer may have returned rewrites; "
...
_THE_WORK = (
    ...
    "1. MERGE AND DEDUPE. Two reviewers reaching the same trouble from different sides is "
    ...
    "2. ATTRIBUTE. `reviewer` is which of the six raised it: `grounding`, `data_defects`, "
    ...
    "reviewer that raised it: `data`, `choice`, `omission`, `coverage`, `semantic`. The "
    "sixth kind, `gap`, is yours to make: every phrase the grounding reviewer landed on "
    ...
    "and `cost` come across from the reviewer's entry. Fix a `grounding_index` that names "
    ...
    "`settled`) as the reviewer set them unless the pool plainly contradicts them. Carry "
    "`raised_by` word for word; it is the reviewer's own account of who prompted it.\n"
...
_BACKING = (
    ...
    "  A backing is a PHRASE of a pool line, never a lone digit, a separator or a space: "
    "the pool prints those everywhere, and a copy of one names nothing.\n"
    "  The pool is the only source. Not the journalist's sentence: the claim is the thing "
    "under attack and cannot be its own footing. And not the reviewer's `evidence` either: "
    "an reviewer has no tools and cannot count anything, so a figure in its `evidence` is "
    ...
    "off the line itself. A figure you find only in an reviewer's sentence and nowhere in "
    ...
    "figure.\n"
    "  THE CAP ON WHAT IT MOVES. A challenge of one of the five priced kinds (`data`, "
    "`choice`, `omission`, `coverage`, `semantic`) whose moved value or bound the pool "
    "does not print weighs at most 1 for what it moves — and that holds whether its "
    "`moves` reads `moves` or `unpriced`. Those two words settle what the page shows the "
    "reader, who moves being known against nothing about it being known, and never the "
    "weight. What carries a challenge past the cap is not a value but missing footing: "
    "one showing the sentence has nothing under it is weighed 3 on that, as the worked "
    "example below is. A `gap` is the exception and is never capped. A phrase landing on "
...
_SEVERITY = (
    ...
    "moved value, or the bound, printed on a line.\n"
    ...
    "that shows it does not move, printed on a line.\n"
...
ORCHESTRATOR_EXAMPLE_JSON = """{
  "challenges": [
    ...
    },
    {
      ...
      "evidence": "The grounding reviewer landed this phrase on nothing. The run starts at the `cases` stage, whose STAGES line reads reads: none, and no stage, column or term under it names a guard, so the run cannot count guards at all.",
      "backing": "reads: none",
      "evidence_refs": [
...
_EXAMPLE = (
    ...
    "reviewer's `evidence` named two lines of the pool, and these are those lines:\n"
    + ORCHESTRATOR_EXAMPLE_POOL_LINES + "\n"
    "You weigh it 3, not 2. It does turn the figure into a bound, which is rubric row 2 — "
    "but that bound is on no line of the pool, as this challenge's own `evidence` says, "
    "so the cap holds that route to 1. What carries it to 3 is the footing: the sentence "
    "asserts an outcome for every case, and the column that holds the outcome prints "
    "`7061 filled` out of `rows 9806`, so the footing the sentence needs is missing "
    "rather than merely moved. The backing is two words copied "
    "off the INPUT COLUMNS line, "
    "not out of the reviewer's sentence, and the ` · ` the line puts after them is no "
    "obstacle.\n"
    "The second entry is the `gap`, and no reviewer raised it: the grounding reviewer "
