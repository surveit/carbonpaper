...
class ReviewedField:
    ...
    template renders as a `<select>` over `options`. `description` is None where
    neither the target nor the source column declares one — never invented."""
...
class QueuedColumn:
    ...
    None where the input edge declares none — unknowable, never inferred from
    the values, and rendered as no tooltip at all."""
...
def build_queue_page(
    ...
    """Under `drift` the recorded decisions describe a different column set, so
    the page carries the notes and no rows at all."""
...
def _as_option_text(value: object) -> str:
    """A bool carries the same "true"/"false" spelling the form's options and
    the reviewed-value parser use; everything else compares by its own text."""
