...
def tail_start_seq(project_id: str, run_id: str, tail: int, stage: str | None = None) -> int:
    ...
    # Walked from the NEWEST chunk back, stopping once `tail` of them are in
    # hand. Reading forward from seq 0 meant every panel that opened parsed the
    # whole log — 333ms of blocking work on a 100k-event run, per open, per
    # panel, and the run page carries two.
    exhausted = True
    for events in read_events_backward(project_id, run_id):
        seen[0:0] = select_stage_events(events, stage)
        if tail > 0 and len(seen) >= tail:
            exhausted = False
            break
        if tail <= 0 and seen:
            exhausted = False
