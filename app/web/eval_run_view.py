...
def build_eval_rows(result_path: Path, dataset: TableRef | None, project: str) -> EvalRowsView:
    try:
    ...
    inputs, input_error = _read_dataset_inputs(dataset, project, checks, len(result))
...
def _read_dataset_inputs(
    dataset: TableRef | None, project: str, checks: list[str], scored_rows: int
) -> tuple[pd.DataFrame | None, str | None]:
    if dataset is None:
        return None, "this eval has no dataset attached, so the scored inputs can't be shown"
    try:
        frame = read_table_ref(project, dataset)
