...
def _render_turn_note(context: BaseModel) -> str:
    assert isinstance(context, EditingContext)
    if not context.page:
        return ""
    return _TURN_PAGE_NOTE.format(url=context.base_url.rstrip("/") + context.page)


# Said every turn: the rail carries one conversation across every page.
_TURN_PAGE_NOTE = """\
The reader is looking at {url} right now. Answer about what is on that page when they say \
"this" or "here", and read the ids out of the address rather than asking them for one."""
...
CONFIG = AgentConfig(
    ...
    # Where they are now goes in the TURN, not here: this string is the cached prefix.
    render_turn_note=_render_turn_note,
