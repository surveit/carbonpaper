...
_SYSTEM_PROMPT = (
    ...
    "lack it, ask.\n\n"
    "Every stage you write declares its schemas or it is REFUSED: each entry in "
    "`inputs` carries a `schema` naming at least one column (copy the upstream "
    "stage's output_schema, or project it down to the columns this stage reads), and "
    "every stage declares an `output_schema` of at least one column — only `publish` "
    "is exempt, because it emits files rather than a table. At run time a declared "
    "column the stage does not produce fails the stage and blocks everything "
    "downstream.\n\n"
