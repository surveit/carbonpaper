...
def add_stages_reporting_drops(
    project_id: str, draft_id: str, stages: Sequence[SubmittedStage]
) -> dict[str, Any]:
    result = project_service.add_stages_reporting_outcome(project_id, draft_id, stages)
...
def edit_stages_reporting_drops(
    project_id: str, draft_id: str, edits: Sequence[StageEdit]
) -> EditedStages:
    ...
    result = project_service.edit_stages(project_id, draft_id, trimmed)
    return EditedStages(
        ok=result.ok,
