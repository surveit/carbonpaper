...
class WorkflowVersion(PersistedModel):
    """`id` is the composite `{project}/{version_id}`; `published` is a signal, not a gate on running."""
...
def find_latest_review_guide(project: str, version_id: str) -> ReviewGuide | None:
    # An autogenerated id has no project prefix, so this reads every stored guide.
    guides = [
...
def delete_project_versions(project_id: str) -> None:
    """Deletes the guides with them, and validates neither: an unreadable document must still go."""
    store = get_store()
    for doc_id in store.list_ids("workflow_version", f"{project_id}/"):
        store.delete("workflow_version", doc_id)
    for doc_id in _find_project_guide_ids(project_id):
        store.delete("review_guide", doc_id)


def _find_project_guide_ids(project_id: str) -> list[str]:
    # An autogenerated id has no project prefix, so this reads every stored guide.
    return [
        doc_id
        for doc_id, data in get_store().read_all("review_guide")
