...
def _load_one_input(project: str, input_id: StageId, reads: TableSchema) -> InputRows:
    ...
    frame = _narrow_to_reads(read_stage_output(project, run_id, input_id), columns,
    ...
    return InputRows(
        input_id=input_id, run_id=run_id, frame=frame,
        profile=profile_frame(frame, columns, max_values=PROFILE_VALUES),
    )


def _narrow_to_reads(
    frame: pd.DataFrame, columns: list[str], *, input_id: StageId, run_id: str
) -> pd.DataFrame:
    """Positional index, so a stored selection means the same row on every re-read."""
    missing = [name for name in columns if name not in frame.columns]
    if missing:
        raise NoRowsToSelectFrom(
            f"`{input_id}` in run {run_id} holds no column {missing} that this step "
            f"says it reads — it holds {sorted(str(name) for name in frame.columns)}. "
