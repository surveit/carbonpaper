...
The WORKFLOW is NEVER generated here. It is authored incrementally — one validated stage at a
time — through app.services.stage_edit, driven by an MCP client (app.mcp) or the editing agent
(app.agents). There is deliberately no whole-workflow generation entry point: nothing in this
module can overwrite or reset a workflow that already has stages.
...
def start_generation(project_dir: Path, *, document: str, model: str) -> str:
    ...
    when it ends); on a valid submission its schemas are written. The workflow is NOT built here
    — the create-flow stops at the data model so it can be reviewed/approved, and the stages are
    then authored one at a time. Must be called from the server event loop — the underlying turn
    is started there."""
...
def _finish_data_model(project_dir: Path, answer: SchemaLibrary | None) -> None:
    ...
    workflow's stages are authored later, one at a time, against the reviewed data model. A
    failed submission was already streamed to the live turn; there is nothing to persist."""
    if answer is None:
        return
