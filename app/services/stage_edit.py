...
def _current_specs(project: str) -> dict[str, dict]:
    ...
    A workflow may legitimately be EMPTY — a project holds no stages until its
    first one is added — and that reads as ``{}``, the starting point the first
    ``add_stage_spec`` builds on (and against which every stage id is unknown, so
    edit/patch/remove raise FileNotFoundError).

    A workflow that HAS stages must load cleanly: it is read through the strict
    ...
    are told apart by whether a working copy exists at all — never by interpreting
    a load failure as emptiness."""
    if not has_working_copy(project) or not read_stage_specs(project):
        return {}
    return stage_specs_by_id(project)
...
def _apply(project: str, specs: dict[str, dict], stage_id: str, candidate: dict) -> EditStageResult:
    ...
    `validate_workflow_draft` the loader enforces), and only if clean store the
    whole list. Returns issues and writes nothing otherwise."""
