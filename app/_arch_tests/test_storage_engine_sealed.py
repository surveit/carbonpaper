...
``app/core/sqlite_store.py`` owns the SQLite backend; every record persists through
...
def test_sqlite3_imported_only_by_the_store() -> None:
    ...
    assert not offenders, (
        "sqlite3 must be imported only by app/core/sqlite_store.py (the concrete "
        "backend); every other module persists via PersistedModel / FrameStore:\n  "
        + "\n  ".join(offenders)
    )
