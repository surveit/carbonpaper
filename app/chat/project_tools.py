"""project_tools.py — in-process tools the editing agent calls to read and edit a
project's workflow. `make_project_tools(current_project, examples_dir=...)` returns
callables closed over the workspace's examples dir. Each read/write tool takes an
explicit `project_id` and resolves `examples_dir / project_id` per call, so one
agent can act across projects; `get_current_project` returns the project the
session was opened on (call it first, then pass its value as `project_id`).
Each tool calls a service directly — no HTTP.
