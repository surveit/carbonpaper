...
def _score_run(
    ...
    dataset = read_table_ref(project, config.table)
    run_id = _mint_run_id()
    ...
    try:
        outputs = run_subset(
            ...
            injected_outputs=_build_injected_outputs(project, config, override, target, dataset),
            project=project, workflow_version=version)
...
def _build_injected_outputs(
    project: str, config: EvalConfig, override: WorkflowStage, target: WorkflowStage,
    ...
    for ref in config.reference_overrides:
        outputs[ref.stage_id] = read_table_ref(project, ref.table)
    return outputs
