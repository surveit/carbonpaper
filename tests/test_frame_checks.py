...
def _rows(columns: dict) -> list[dict]:
    """Column-wise for readability; the checks take rows."""
    names = list(columns)
    return [dict(zip(names, cells)) for cells in zip(*(columns[n] for n in names))]


def test_a_clean_frame_has_no_violations():
    rows = _rows({"id": ["a", "b"], "n": [1, 2]})
    assert find_frame_violations(rows) == []
...
def test_rows_differing_anywhere_are_not_duplicates():
    ...
    rows = _rows({"id": ["a", "a"], "n": [1, 2]})
    assert find_duplicate_row_violations(rows) == []


def test_a_repeated_row_names_its_0_based_positions():
    rows = _rows({"id": ["a", "b", "a"], "n": [1, 2, 1]})
    violation = find_duplicate_row_violations(rows)[0]
...
def test_an_empty_frame_has_no_duplicates():
    assert find_duplicate_row_violations([]) == []
...
def test_groups_past_the_named_limit_are_counted_off_rather_than_listed():
    ...
    rows = _rows({"id": [letter for letter in "aabbccddeeff"]})
    message = find_duplicate_row_violations(rows)[0].message
    assert message.count("rows [") == 5
    assert "(+1 more group(s))" in message


def test_find_frame_violations_reports_the_duplicate_row_rule():
    rows = _rows({"id": ["a", "a"], "n": [1, 1]})
    messages = [v.message for v in find_frame_violations(rows)]
