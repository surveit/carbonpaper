...
# Stage requires a schema on every input and a signature. The only producer
# these fixtures build is `load`, so an input edge always carries `load`'s
# resolved output; the llm_transform's signature adds `score` on top of it, so
# it stays strictly 1:1.
