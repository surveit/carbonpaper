...
# ─── the removed one-shot chain: no whole-workflow writer on this surface ─────

def test_no_whole_workflow_regeneration_tool_is_exposed(examples_root: Path) -> None:
    """The editing agent can grow and repair a workflow, never replace it. A tool
    that rebuilt the whole workflow from a conversation (the old compile_workflow)
    could overwrite reviewed stages wholesale, which #243 removed outright."""
    names = {tool.__name__ for tool in _tools("alpha")}
    assert "compile_workflow" not in names
