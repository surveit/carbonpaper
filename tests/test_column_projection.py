...
dropped and recorded on the stage's accumulation, not silently discarded."""
...
def test_llm_transform_drops_undeclared_columns_including_former_hardcoded_ids(monkeypatch):
    ...
    dropped = accumulation_of(out).dropped_columns
...
def test_llm_transform_declared_input_column_rides_through(monkeypatch):
    ...
    assert not accumulation_of(out).dropped_columns           # nothing undeclared
