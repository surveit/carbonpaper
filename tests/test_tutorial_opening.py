...
and fed back into the system prompt — so the reader reads it and the model knows it
said it.
"""
...
def test_the_tour_stores_its_greeting_as_its_one_opening_turn() -> None:
    assert _stored_greeting(_open_the_tour()) == TUTORIAL_OPENING_MESSAGE


def test_the_greeting_closes_on_the_question_and_nothing_after_it() -> None:
    """Its failure mode: a closing gloss tacked on after the question that should end it."""
    greeting = _stored_greeting(_open_the_tour())

    assert greeting.endswith("Ready to get started? I'll seed a sample investigation and walk you through it.")
