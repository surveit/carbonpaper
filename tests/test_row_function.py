...
# Every frame below is a single int column `x`; `adds` names what the `code`
# under test returns beyond it.
_X_COLUMN = [{"name": "x", "type": "int", "nullable": True}]


def _stage(code, inputs=("src",), adds=()):
    return parse_stage({
        "id": "t", "name": "t", "type": "python_row_function",
        "inputs": [{"id": i, "schema": {"columns": _X_COLUMN}} for i in inputs],
        "signature": {"form": "extends", "adds": list(adds)},
...
def test_row_function_maps_per_row():
    ...
    out = _run(_stage(code, adds=[{"name": "y", "type": "int", "nullable": True}]),
               {"src": df})
