"""Every reviewer is told its place, and the orchestrator is told the guard is verbatim."""
...
from app.reviewer import attackers_prompt, orchestrator_prompt
...
_EXAMPLE_JSON: dict[str, tuple[str, type[BaseModel]]] = {
    ...
    "orchestrator": (orchestrator_prompt.ORCHESTRATOR_EXAMPLE_JSON, OrchestratorAnswer),
}

# The reviewer cannot split rows or sum a column, so its example must not model one.
...
def test_the_orchestrator_is_told_the_pool_is_the_only_source_of_a_backing() -> None:
    ...
    assert "not the reviewer's `evidence` either" in _ORCHESTRATOR
...
def test_the_orchestrator_spells_every_value_it_must_choose_among() -> None:
    for value in [*Reviewer, *ChallengeKind, *Moves, *Cost]:
...
def test_the_orchestrator_example_backing_is_copied_off_a_pool_line() -> None:
    draft = OrchestratorAnswer.model_validate(
...
def test_the_worked_example_shows_a_gap_weighed_beside_a_priced_challenge() -> None:
    ...
    assert gap.reviewer == Reviewer.grounding and gap.severity == 3
