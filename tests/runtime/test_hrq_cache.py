...
def _read_fingerprints(run_id: str, stage_id: str, project: str = PROJECT) -> dict:
    """What the halted queue stage stored for this run."""
...
def _halt_and_read_snapshot(
    ...
    return (pd.read_parquet(queue_path),
            _read_fingerprints(ctx.identity.run_id, queue_path.stem))
...
def test_queue_stats_count_every_row_the_reviewer_answered(tmp_path):
    ...
    queue_path = exc_info.value.queue_path
    snapshot = pd.read_parquet(queue_path)
    fingerprints = _read_fingerprints("run1", queue_path.stem)
...
def test_resume_reattaches_cached_decisions_written_via_the_seam(tmp_path):
    ...
    assert len(snapshot) == 2
    fingerprints = _read_fingerprints(run_id, "review", project_dir.name)
