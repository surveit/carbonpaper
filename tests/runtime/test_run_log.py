...
def test_a_run_writes_its_lifecycle_spine_to_the_run_dir(tmp_path):
    ...
    executor — so a run always leaves its event log behind, terminated
...
def _resumable_log(run_id: str, stage: str) -> None:
    """Log one stage_start + the run_done marker, appending to what is there."""
    log = RunLog(PROJECT, run_id)
...
def test_a_resumed_log_keeps_seq_equal_to_the_line_index(tmp_path):
    """resume_run opens a second RunLog on the same run; seq is the event's own
    position, so it stays strictly monotonic across any number of resumes."""
