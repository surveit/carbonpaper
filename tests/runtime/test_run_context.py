...
def test_production_context_rejects_queue_auto_approve(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValidationError, match="non-production bypass"):
        RunContext(
            mode="production",
            repo_root=tmp_path,
...
def test_non_production_context_allows_queue_auto_approve(tmp_path: Path) -> None:
...
def test_for_production_stamps_mode_and_grants_scope(tmp_path: Path) -> None:
    ctx = RunContext.for_production(tmp_path, tmp_path / "run", "proj", "r1")
    assert ctx.mode == "production"
    assert ctx.identity == RunIdentity(project="proj", run_id="r1")
    assert ctx.stage_cache is not None


def test_for_non_production_allows_none_paths() -> None:
