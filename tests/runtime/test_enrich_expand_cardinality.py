...
def test_output_is_subject_columns_plus_enrich_with_only():
    # The handler narrows the reference before merging, so `extra` cannot leak.
    out = handle_expand(
        _join_stage("expand"),
...
def test_a_brought_column_lands_under_its_authored_name():
    # An authored rename (`z: z2`), never a silent suffix.
    stage = parse_stage({
...
def test_a_right_key_sharing_a_subject_columns_name_is_dropped():
    # The reference's own `x` shares the subject's name but is un-brought.
    stage = parse_stage({
        "id": "m", "name": "Join", "type": "enrich",
        "inputs": [{"id": "subject", "schema": _SUBJECT},
                   {"id": "reference",
