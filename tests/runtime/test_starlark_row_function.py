...
def test_maps_the_function_over_every_row():
    ...
    out = _handler().execute(place_stage(stage), as_inputs({"src": pd.DataFrame({"n": [1, 2, 3]})}), make_run_context())
...
def test_starlark_cannot_reach_the_python_object_graph():
    ...
    with pytest.raises(starlark.StarlarkError, match="has no attribute `__class__`"):
        _handler().execute(place_stage(stage), as_inputs({"src": pd.DataFrame({"n": [1]})}), make_run_context())
...
def test_a_row_with_a_datetime_is_marshalled_before_starlark_sees_it():
    ...
    out = _handler().execute(
        place_stage(stage), as_inputs({"src": pd.DataFrame({"ts": [pd.Timestamp("2024-01-02T03:04:05")]})}),
        make_run_context(),
    )
...
def test_an_empty_function_name_falls_back_to_transform():
    ...
    out = _handler().execute(place_stage(stage), as_inputs({"src": pd.DataFrame({"n": [5]})}), make_run_context())
