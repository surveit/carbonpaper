...
def test_a_batched_stage_stops_rather_than_failing_every_chunk(exhausted_account):
    with pytest.raises(AccountLimitReached):
        lt.run_llm_batches(_placed(batch_size=2), as_inputs({"load": _ROWS.copy()}),
                           make_run_context(), parallelism=1, positions=list(range(12)))
    # One chunk asked, at parallelism 1 — the remaining five never ran.
