...
# column), so the classify diff has both to show; every input column flows.
_CLASSIFY_CODE = (
    "def transform(row):\n"
...
def _seed_compiled(pdir: Path, data_path: Path, routes_path: Path) -> None:
    ...
    stages = [
        ...
        ("05_route.json", {
            ...
            "signature": {
                "form": "extends",
                "reads": [{"input": CLASSIFY_ID, "columns": [
                               {"name": "name", "type": "str", "nullable": True}]},
                          {"input": ROUTES_ID, "columns": [
                               {"name": "name", "type": "str", "nullable": True}]}],
...
def test_the_rail_tallies_what_the_stage_did_in_one_line(run_ctx) -> None:
    ...
    # classify adds `label` and uppercases one name; every input column flows.
