...
def test_a_new_column_reads_as_added():
    diff = build_row_diff({"a": 1, "score": 5}, {"a": 1}, writes_every_column=False)
...
def test_a_changed_cell_carries_the_value_it_replaced():
    diff = build_row_diff({"a": "new"}, {"a": "old"}, writes_every_column=False)
...
def test_a_dropped_column_shows_the_value_the_step_discarded():
    diff = build_row_diff({"a": 1}, {"a": 1, "note": "gone"}, writes_every_column=False)
...
def test_a_null_renders_empty_rather_than_the_word_none():
    diff = build_row_diff({"a": None}, None, writes_every_column=True)
    assert diff.columns[0].text == ""


def test_a_bool_renders_as_python_prints_it_matching_the_stage_table():
    diff = build_row_diff({"flag": False}, None, writes_every_column=True)
    assert diff.columns[0].text == "False"


def test_a_replacing_stages_columns_are_all_new_with_no_parent_to_compare():
    diff = build_row_diff({"a": 1, "b": 2}, None, writes_every_column=True)
    assert set(_states(diff).values()) == {CellDiffState.added}
    assert diff.added == 2


def test_an_untraced_upstream_claims_nothing_about_what_the_step_added():
    diff = build_row_diff({"a": 1, "b": 2}, None, writes_every_column=False)
...
def test_a_difference_only_the_types_can_see_is_not_marked():
    diff = build_row_diff({"a": "1"}, {"a": 1}, writes_every_column=False)  # both render "1"
    assert diff.columns[0].state is CellDiffState.carried


def test_a_difference_the_reader_can_see_is_marked():
    diff = build_row_diff({"a": 1}, {"a": 1.0}, writes_every_column=False)  # "1" against "1.0"
    assert diff.columns[0].state is CellDiffState.changed


def test_a_carried_column_the_stage_read_is_not_inert():
    read = build_row_diff({"a": "x"}, {"a": "x"}, writes_every_column=False, read=frozenset({"a"}))
    unread = build_row_diff({"a": "x"}, {"a": "x"}, writes_every_column=False)
...
def test_parsing_a_grouped_string_to_a_number_still_reads_as_changed():
    diff = build_row_diff({"paid": 10000}, {"paid": "10,000"}, writes_every_column=False)
    column = diff.columns[0]
