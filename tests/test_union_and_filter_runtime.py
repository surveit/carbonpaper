...
def test_union_concatenates_two_inputs_in_declared_order(tmp_path):
    ...
    outputs = run_subset(
        ...
        stage_ids=["left", "right", "u"], run_dir=tmp_path / "runs" / "r1", project=(tmp_path / "runs" / "r1").parent.parent.name)
...
def test_filter_rows_keeps_true_rows_in_order_with_columns_unchanged(tmp_path):
    ...
    outputs = run_subset(
        workflow, injected_outputs={},
        stage_ids=["src", "f"], run_dir=tmp_path / "runs" / "r2", project=(tmp_path / "runs" / "r2").parent.parent.name)
...
def test_a_filter_that_keeps_nothing_still_feeds_its_downstream_a_valid_frame(tmp_path):
    ...
    outputs = run_subset(
        ...
        project=(tmp_path / "runs" / "r_empty").parent.parent.name)
...
def test_filter_rows_non_bool_return_is_a_loud_error(tmp_path):
    ...
    with pytest.raises(SubsetRunError) as exc_info:
        run_subset(
            workflow, injected_outputs={},
            stage_ids=["src", "f"], run_dir=tmp_path / "runs" / "r3", project=(tmp_path / "runs" / "r3").parent.parent.name)
...
def test_trace_walks_through_filter_rows_to_the_right_source_row(tmp_path):
    ...
    run_subset(
        workflow, injected_outputs={},
        stage_ids=["src", "f"], run_dir=run_dir, project=(run_dir).parent.parent.name)
...
def test_trace_walks_through_union_to_the_right_source_row_in_the_right_input(tmp_path):
    ...
    run_subset(
        workflow, injected_outputs={},
        stage_ids=["left", "right", "u"], run_dir=run_dir, project=(run_dir).parent.parent.name)
...
def test_trace_follows_lineage_after_a_limit_caps_what_the_filter_reads(tmp_path):
    ...
    outputs = run_subset(
        workflow, injected_outputs={},
        stage_ids=["src", "f"], run_dir=run_dir, params=RunParameters(limits={"f": 2}), project=(run_dir).parent.parent.name)
...
def test_a_row_mapper_that_may_not_drop_still_rejects_a_none_row(tmp_path):
    ...
    with pytest.raises(SubsetRunError) as exc_info:
        run_subset(
            ...
            project=(tmp_path / "runs" / "none_row").parent.parent.name)
