...
def test_a_plain_extends_signature_ok():
    stage = parse_stage(_filter_stage())
    assert [c.name for c in stage.resolve_output_schema().columns] == ["a", "b"]


def test_a_signature_add_is_rejected():
    with pytest.raises(ValidationError, match="reads only, never adds or rewrites"):
        parse_stage(_filter_stage(signature={
            "form": "extends",
            "adds": [{"name": "c", "type": "str", "nullable": True}]}))


def test_a_signature_rewrite_is_rejected():
    with pytest.raises(ValidationError, match="reads only, never adds or rewrites"):
        parse_stage(_filter_stage(signature={
            "form": "extends",
            "reads": [{"input": "src", "columns": [{"name": "b", "type": "int", "nullable": True}]}],
            "rewrites": [{"name": "b", "type": "str", "nullable": True}]}))
