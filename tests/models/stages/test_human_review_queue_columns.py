...
def _stage_spec(*, queue=None, input_columns=None, adds=None):
    return {
        "id": "wc", "type": "human_review_queue", "name": "wc",
        "inputs": [{"id": "src", "schema": {"columns": input_columns or _INPUT_COLUMNS}}],
        "signature": {"form": "extends", "adds": adds if adds is not None else _ADDED_COLUMNS},
...
def test_a_non_scalar_reviewed_source_is_rejected():
    ...
    with pytest.raises(ValidationError, match="evidence"):
        parse_stage(_stage_spec(
            ...
            input_columns=input_columns, adds=adds,
        ))


# ── 3. reviewed target columns among the signature adds ──────────────────────


def test_a_reviewed_target_the_signature_does_not_add_is_rejected():
...
def test_a_reviewed_target_of_the_wrong_type_is_rejected():
    adds = [
        ...
        for c in _ADDED_COLUMNS
    ]
    with pytest.raises(ValidationError, match="human_score"):
        parse_stage(_stage_spec(adds=adds))
...
# ── 4. the review-record columns among the signature adds ────────────────────


def test_a_review_record_column_the_signature_does_not_add_is_rejected():
...
def test_a_declared_notes_column_must_be_among_the_adds():
    with pytest.raises(ValidationError, match="review_notes"):
        parse_stage(_stage_spec(queue={"review_notes_column": "review_notes"}))


def test_a_declared_notes_column_among_the_adds_is_clean():
    stage = parse_stage(_stage_spec(
        queue={"review_notes_column": "review_notes"},
        adds=_ADDED_COLUMNS + [{"name": "review_notes", "type": "str", "nullable": True}],
    ))
    assert find_queue_column_issues(stage) == []
...
def test_a_non_nullable_verdict_column_is_clean():
    ...
    stage = parse_stage(_stage_spec(adds=adds))
    assert find_queue_column_issues(stage) == []
...
def test_a_review_record_column_that_the_input_already_declares_is_rejected():
    # `decision` is added as `str`, so rule 4 stays silent and only this rule can fire.
...
def test_a_review_record_name_reused_as_a_reviewed_target_is_rejected():
    # The source is `str` non-null and `decision` is added as `str`, so rules 3 and 4
    # stay silent and only this rule can fire.
    with pytest.raises(ValidationError, match="named more than once"):
        parse_stage(_stage_spec(
