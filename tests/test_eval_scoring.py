...
def test_exact_metric_counts_matching_rows():
    override = _stage("ov", [{"name": "doc_id", "type": "str"}])
    target = _stage("tg", [{"name": "doc_id", "type": "str"}, {"name": "label", "type": "str"}])
...
def test_abs_tol_metric_uses_tolerance():
    override = _stage("ov", [{"name": "k", "type": "str"}])
    target = _stage("tg", [{"name": "k", "type": "str"}, {"name": "amt", "type": "float"}])
...
def test_sign_metric_compares_sign_only():
    override = _stage("ov", [{"name": "k", "type": "str"}])
    target = _stage("tg", [{"name": "k", "type": "str"}, {"name": "delta", "type": "float"}])
...
def test_row_count_mismatch_raises_grain_violation():
    override = _stage("ov", [{"name": "k", "type": "str"}])
    target = _stage("tg", [{"name": "k", "type": "str"}, {"name": "label", "type": "str"}])
    config = _config([ExpectedOutput(output_column="label", metric="exact")])
...
def test_checked_column_clashing_with_override_is_read_from_output_prefixed_column():
    ...
    `override.<name>`."""
    override = _stage("ov", [{"name": "label", "type": "str"}])   # override emits `label`
    target = _stage("tg", [{"name": "label", "type": "str"}])     # target also emits `label`
    config = _config([ExpectedOutput(output_column="label", metric="exact")])
