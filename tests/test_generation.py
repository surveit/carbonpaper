...
def test_start_workflow_generation_delegates_to_the_bridge_grounded(
    ...
    def fake_bridge(*, document, project_name, model, data_model, on_answer, post_validate=None):
        captured.update(
            document=document, project_name=project_name, model=model,
            data_model=data_model, on_answer=on_answer, post_validate=post_validate,
        )
        return "sess-xyz"
