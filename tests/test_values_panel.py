...
def test_the_cited_cell_is_marked_and_not_its_whole_column(run_id):
    page = TestClient(app).get(
        f"/project/{PROJECT}/runs/{run_id}/values/panel"
        "?stage=by_portfolio&row=1&column=total_amount")
    assert page.text.count("vu-cell-cited") == 1


def test_every_sheet_names_where_its_rows_sit_in_the_frame(run_id):
    # G-009 is the sixth east row, and a reader counts it as 6.
    page = TestClient(app).get(
        f"/project/{PROJECT}/runs/{run_id}/values/panel"
