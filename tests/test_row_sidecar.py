...
from app.runtime.errors import RowSidecarLengthMismatch
from app.runtime.lineage import RowLineage, RowParent
from app.runtime.row_sidecar import (
    read_row_sidecar,
    resolve_row_sidecar_path,
    write_row_sidecar,
)
...
def test_both_halves_ride_in_one_file(tmp_path) -> None:
    ...
    write_row_sidecar(run_dir, "kept", _KEPT, _TAKEN)

    sidecar = read_row_sidecar(run_dir, "kept")
...
def test_a_branches_only_stage_reports_no_lineage_rather_than_no_parents(tmp_path) -> None:
    ...
    write_row_sidecar(run_dir, "tier", None, _TAKEN)

    sidecar = read_row_sidecar(run_dir, "tier")
...
def test_a_lineage_only_stage_reports_no_branches(tmp_path) -> None:
    run_dir = _outputs(tmp_path)
    write_row_sidecar(run_dir, "kept", _KEPT, None)
...
def test_a_stage_that_recorded_neither_writes_nothing(tmp_path) -> None:
    run_dir = _outputs(tmp_path)
    write_row_sidecar(run_dir, "passthrough", None, None)

    assert not resolve_row_sidecar_path(run_dir, "passthrough").exists()
    assert read_row_sidecar(run_dir, "passthrough") == read_row_sidecar(run_dir, "absent")


def test_an_empty_stage_records_zero_rows_rather_than_nothing(tmp_path) -> None:
    run_dir = _outputs(tmp_path)
    write_row_sidecar(run_dir, "kept", RowLineage([]), RowBranches([]))
...
def test_halves_of_different_lengths_are_refused(tmp_path) -> None:
    run_dir = _outputs(tmp_path)
    with pytest.raises(RowSidecarLengthMismatch, match="2 row"):
        write_row_sidecar(run_dir, "kept", _KEPT, RowBranches([("should_include/0:if",)]))
