...
def test_error_blocks_transitive_downstream_in_a_chain(tmp_path):
    ...
    manifest = run_prepared(prepare_run(tmp_path, *pinned_stages(tmp_path)))
...
def test_resume_pops_stale_halted_at_before_re_executing(tmp_path, monkeypatch):
    ...
    halted = run_prepared(prepare_run(tmp_path, *pinned_stages(tmp_path)))
    ...
    runner.resume_run(tmp_path, halted["run_id"],
...
def test_row_error_stage_blocks_downstream_and_resume_is_not_stale(tmp_path, monkeypatch):
    ...
    first = run_prepared(prepare_run(tmp_path, *pinned_stages(tmp_path)))
    ...
    failing["text"] = None
    resumed = runner.resume_run(tmp_path, first["run_id"],
...
def test_resume_after_error_reruns_the_errored_stage_and_its_downstream(tmp_path):
    ...
    prep = prepare_run(tmp_path, *pinned_stages(tmp_path))  # preflight hashes the valid file
...
def test_resume_refuses_stages_belonging_to_another_version(tmp_path):
    ...
    with pytest.raises(ValueError, match="pinned to workflow version"):
        runner.resume_run(project_dir, halted["run_id"], stages, "20990101T000000")
