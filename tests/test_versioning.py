...
app.services.drafts and app.web.loading all depend on by signature.
...
a version) lives in test_runner.py; the editing agent's draft→version flow lives
in test_drafts.py / test_project_tools.py; eval-pinning lives in
test_eval_runner.py / test_eval_store.py. This file exercises versioning.py's own
contract directly.
"""
