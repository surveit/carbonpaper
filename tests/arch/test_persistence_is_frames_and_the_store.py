...
_ALLOWED_WRITERS: dict[str, str] = {
    ...
    "app/web/routers/admin.py": "writes a project export bundle the user downloads",

    # 3. A file the user handed us. Raw bytes we did not author and must not
    # reinterpret — an input CSV, an eval dataset.
    "app/web/loading.py": "receives an uploaded run-input file",
    "app/evals/store.py": "receives an uploaded eval dataset",

    # Not app state at all: a dev script that regenerates a committed fixture.
    "app/seeds/capture_lobbying.py": "regenerates a committed test fixture",
...
def find_file_writes(tree: ast.AST) -> list[tuple[int, str]]:
    ...
    return sorted(
        ...
        and (name := _called_name(node.func)) in _WRITE_CALLS
    )
