"""One reader, three policies. `app.runtime.manifest.list_run_entries` reads every
stored run at both levels (raw payload and typed model); what an unreadable or
untypable one MEANS is each caller's own answer, and the three answers differ on
purpose. Pinned here side by side, because folding them onto one policy silently
changes what a page shows.
"""
...
from run_seed import store_manifest_text
...
def _write_run(pdir: Path, run_id: str, manifest_text: str) -> None:
    store_manifest_text(pdir, run_id, manifest_text)
...
# ─── A stored payload whose bytes are not JSON ───────────────────────────────
...
def test_the_runs_summary_counts_an_unparseable_manifest_as_a_corrupt_run(project_dir: Path):
    ...
    summary = project_service._runs_summary(project_dir.name)
...
# ─── A payload that is JSON, but not a shape RunManifest accepts ─────────────
...
def test_the_runs_index_lists_a_test_run_the_two_counting_readers_exclude(project_dir: Path):
    ...
    assert project_service._runs_summary(project_dir.name).n == 0
    card, = loading.list_projects()
    assert card["n_runs"] == 0
