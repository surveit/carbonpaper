...
def test_mid_stage_cancel_marks_the_running_stage_cancelled_not_pending(tmp_path, monkeypatch):
    ...
    stages: execution.py's is_cancelled binding is forced True so the row
    driver raises RunCancelled mid-fan-out, while runner.py's own is_cancelled
    binding stays real (never requested via request_cancel) so 'score' starts
    rather than being skipped by the between-stage checkpoint above. Exercises
    the runner's `except RunCancelled:` branch: the stage that was RUNNING is
    recorded 'cancelled', distinct from a not-yet-started stage's 'pending'."""
    def fake_call_llm(stage_id, llm_config, row, **kw):
        return {"score": 1}

    monkeypatch.setattr(lt, "call_llm", fake_call_llm)
    monkeypatch.setattr(execution, "is_cancelled", lambda project, run_id: True)
