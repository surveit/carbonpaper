...
def test_override_conflict_saves_anyway_and_carries_none_of_their_changes(examples: Path) -> None:
    ...
    [stage] = versioning.load_version_stages(pdir.name, result.version_id)
    assert stage.id == "load"


def test_a_draft_that_claimed_no_base_is_never_a_conflict(examples: Path) -> None:
    pdir = examples / "demo"
    versioning.create_version_from_stages(pdir.name, [_STAGE], message="v1")
    scratch = drafts.create_draft("demo")
    drafts.set_draft_stage("demo", scratch.id, json.dumps(_STAGE))

    result = drafts.save_version("demo", scratch.id, message="from scratch")
