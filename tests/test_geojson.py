"""read_source_geojson flattens a FeatureCollection into a DataFrame (app/core/source_files.py)."""
from __future__ import annotations

import json
from pathlib import Path

from app.core.source_files import read_source_geojson
...
def test_flattens_points_with_lon_lat(tmp_path):
    ...
    df = read_source_geojson(_write(tmp_path, fc))
    assert len(df) == 2
...
def test_empty_feature_collection(tmp_path):
    df = read_source_geojson(_write(tmp_path, {"type": "FeatureCollection", "features": []}))
    assert len(df) == 0
