...
def test_adding_a_code_stage_without_a_summary_is_refused(project):
    result = add_stage_spec(project, SEED_DRAFT, json.dumps(_spec()))
...
def test_adding_a_code_stage_with_a_summary_is_accepted(project):
    result = add_stage_spec(project, SEED_DRAFT, json.dumps(_spec(summary="Passes rows through.")))
    assert result.ok, result.issues


def test_a_blank_summary_does_not_satisfy_the_gate(project):
    result = add_stage_spec(project, SEED_DRAFT, json.dumps(_spec(summary="   ")))
    assert not result.ok
    assert any("summary` is required" in issue for issue in result.issues)


def test_editing_a_summary_away_is_refused(project):
    assert add_stage_spec(project, SEED_DRAFT, json.dumps(_spec(summary="Passes rows through."))).ok
    result = edit_stage_spec(project, SEED_DRAFT, "tag", json.dumps(_spec()))
    assert not result.ok
    assert any("summary` is required" in issue for issue in result.issues)


def test_a_config_only_stage_needs_no_summary(project):
    result = add_stage_spec(project, SEED_DRAFT, json.dumps({
...
def test_omitting_corner_cases_entirely_is_refused(project):
    ...
    result = add_stage_spec(project, SEED_DRAFT, json.dumps(spec))
    assert not result.ok
...
def test_a_summary_over_the_limit_is_refused(project):
    result = add_stage_spec(project, SEED_DRAFT, json.dumps(_spec(summary="x" * (SUMMARY_MAX_CHARS + 1))))
    assert not result.ok
...
def test_a_summary_exactly_at_the_limit_is_accepted(project):
    result = add_stage_spec(project, SEED_DRAFT, json.dumps(_spec(summary="x" * SUMMARY_MAX_CHARS)))
    assert result.ok, result.issues
