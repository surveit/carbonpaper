...
# ── the input schema is a cache, and it can go stale ─────────────────────────

_GREW = {"columns": [{"name": "id", "type": "str", "nullable": True},
                     {"name": "added_later", "type": "int", "nullable": True}]}


def _source(stage_id, schema):
    return m.parse_stage({
        "id": stage_id, "description": "Load", "type": "input_data",
        "connector": {"kind": "file", "params": {"path": "/tmp/x.csv", "format": "csv"}},
        "signature": {"form": "replaces", "produces": schema["columns"]}})


def _reader(stage_id, upstream_id, schema, adds):
    return m.parse_stage({
        "id": stage_id, "description": "Read", "type": "python_row_function",
        "inputs": [{"id": upstream_id, "schema": schema}],
        "signature": {"form": "extends",
                      "reads": [{"input": upstream_id, "columns": schema["columns"]}],
                      "adds": adds},
        "function": {"kind": "inline", "summary": "Passes every row through unchanged.",
                     "code": _CODE},
        "tests": [{"name": "passes_through",
                   "inputs": {upstream_id: [dict.fromkeys(
                       [c["name"] for c in schema["columns"]], "v")]},
                   "expected": [{**dict.fromkeys(
                       [c["name"] for c in schema["columns"]], "v"),
                       **{a["name"]: "v" for a in adds}}]}]})


def test_an_input_schema_that_did_not_follow_its_upstream_warns():
    stages = [_source("up", _GREW),
              _reader("mid", "up", _SCHEMA, [{"name": "tag", "type": "str", "nullable": True}])]

    report = find_workflow_compiler_warnings(stages)

    stale = [w for w in report.warnings if w.kind == "stale_input_schema"]
    assert [w.stage_id for w in stale] == ["mid"]
    assert "added_later" in stale[0].detail


def test_the_drift_is_reported_at_every_stage_that_copied_it():
    """Only the edge that dropped the column is named."""
    mid_output = {"columns": [*_SCHEMA["columns"],
                              {"name": "tag", "type": "str", "nullable": True}]}
    stages = [_source("up", _GREW),
              _reader("mid", "up", _SCHEMA, [{"name": "tag", "type": "str", "nullable": True}]),
              _reader("tail", "mid", mid_output, [{"name": "note", "type": "str", "nullable": True}])]

    kinds = [(w.stage_id, w.kind) for w in find_workflow_compiler_warnings(stages).warnings]

    assert ("mid", "stale_input_schema") in kinds
    # `tail`'s copy matches what `mid` DECLARES, so tail itself is not drifted —
    # which is the point: the omission is invisible from tail, and only the edge
    # that dropped it can be fixed.
    assert ("tail", "stale_input_schema") not in kinds


def test_a_schema_that_matches_its_upstream_is_silent():
    stages = [_source("up", _SCHEMA),
              _reader("mid", "up", _SCHEMA, [{"name": "tag", "type": "str", "nullable": True}])]

    assert not [w for w in find_workflow_compiler_warnings(stages).warnings
                if w.kind == "stale_input_schema"]


def test_stale_drift_never_blocks_a_workflow():
    """It refuses nothing — nobody chose the drift."""
    stages = [_source("up", _GREW),
              _reader("mid", "up", _SCHEMA, [{"name": "tag", "type": "str", "nullable": True}])]
    # An error would stop a workflow loading over a column its author never saw.

    report = find_workflow_compiler_warnings(stages)

    assert report.is_clean
    assert [w.kind for w in report.errors] == []
